...
namespace stellar_dotnet_sdk_test.responses.effects
{
    ...
    public class LiquidityPoolWithdrewEffectResponseTest
    {
        ...
        {
            var json = File.ReadAllText(Path.Combine("responses/effects/", "LiquidityPoolWithdrewEffectResponse/Data.json"));
            var instance = JsonSingleton.GetInstance<EffectResponse>(json);
            var response = (LiquidityPoolWithdrewEffectResponse)instance;
            var clone = new LiquidityPoolWithdrewEffectResponse(response.LiquidityPool, response.ReservesReceived, response.SharesRedeemed);
        ...
        public static void AssertData(EffectResponse instance)
        {
            ...
            Assert.AreEqual(effect.ReservesReceived[1].Asset.CanonicalName(), "TEST:GD5Y3PMKI46MPILDG4OQP4SGFMRNKYEPJVDAPR3P3I2BMZ3O7IX6DB2Y");
